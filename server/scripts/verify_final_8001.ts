...
async function main() {
    ...
    const resultId = 210; // Valid ID found in DB
    const port = 8001;
    ...
    try {
        ...
        // Dummy file
        const dummyFile = path.resolve(process.cwd(), 'tmp/final_test_8001.txt');
        if (!fs.existsSync(dummyFile)) fs.writeFileSync(dummyFile, 'Final verification 8001');

        const cmd = `curl -X POST http://localhost:${port}/api/checkpoints/${resultId}/upload -H "Authorization: Bearer ${token}" -F "file=@${dummyFile}" --fail --silent --show-error -v`;
