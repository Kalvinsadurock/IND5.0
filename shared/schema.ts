...
export const oeeDowntimeEvents = pgTable("oee_downtime_events", {
  ...
  workCenterId: uuid("work_center_id").references(() => platformWorkCenters.id).notNull(),
  downtimeReasonId: uuid("downtime_reason_id"),
...
export const oeeProductionCounts = pgTable("oee_production_counts", {
  ...
  executionId: uuid("execution_id").references(() => workOrderExecutions.id),
...
export const workOrderExecutions = pgTable("work_order_executions", {
  ...
  currentStepId: integer("current_step_id").references(() => processSteps.id),
  workCenterId: uuid("work_center_id").references(() => platformWorkCenters.id),
  executionNumber: varchar("execution_number", { length: 80 }).notNull(),
  status: varchar("status", { length: 40 }).notNull().default("planned"),
  plannedQuantity: decimal("planned_quantity", { precision: 14, scale: 3 }).notNull().default("1"),
  goodQuantity: decimal("good_quantity", { precision: 14, scale: 3 }).notNull().default("0"),
  rejectedQuantity: decimal("rejected_quantity", { precision: 14, scale: 3 }).notNull().default("0"),
  startedAt: timestamp("started_at"),
  endedAt: timestamp("ended_at"),
  createdBy: uuid("created_by").references(() => platformUsers.id),
  updatedBy: uuid("updated_by").references(() => platformUsers.id),
  ...
  index("work_order_executions_tenant_idx").on(table.tenantId),
  index("work_order_executions_work_order_idx").on(table.workOrderId),
  index("work_order_executions_part_idx").on(table.partId),
  index("work_order_executions_status_idx").on(table.tenantId, table.status),
]);

export const inventoryMaterials = pgTable("inventory_materials", {
  id: uuid("id").defaultRandom().primaryKey(),
  tenantId: uuid("tenant_id").references(() => platformTenants.id).notNull(),
  materialCode: varchar("material_code", { length: 80 }).notNull(),
  materialName: varchar("material_name", { length: 180 }).notNull(),
  materialType: varchar("material_type", { length: 80 }).notNull().default("raw_material"),
  baseUom: varchar("base_uom", { length: 24 }).notNull().default("ea"),
  status: varchar("status", { length: 30 }).notNull().default("active"),
  lotControlled: boolean("lot_controlled").notNull().default(false),
  serialControlled: boolean("serial_controlled").notNull().default(false),
  createdBy: uuid("created_by").references(() => platformUsers.id),
  updatedBy: uuid("updated_by").references(() => platformUsers.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => [
  index("inventory_materials_tenant_idx").on(table.tenantId),
  index("inventory_materials_code_idx").on(table.tenantId, table.materialCode),
  index("inventory_materials_type_idx").on(table.tenantId, table.materialType),
]);

export const inventoryStockLedger = pgTable("inventory_stock_ledger", {
  id: uuid("id").defaultRandom().primaryKey(),
  tenantId: uuid("tenant_id").references(() => platformTenants.id).notNull(),
  materialId: uuid("material_id").references(() => inventoryMaterials.id).notNull(),
  workOrderId: uuid("work_order_id").references(() => mesWorkOrders.id),
  executionId: uuid("execution_id").references(() => workOrderExecutions.id),
  ...
  reason: text("reason"),
  postedBy: uuid("posted_by").references(() => platformUsers.id),
  postedAt: timestamp("posted_at").defaultNow().notNull(),
}, (table) => [
  index("inventory_stock_ledger_tenant_idx").on(table.tenantId),
  index("inventory_stock_ledger_material_idx").on(table.materialId),
  index("inventory_stock_ledger_execution_idx").on(table.executionId),
  index("inventory_stock_ledger_work_order_idx").on(table.workOrderId),
]);

export const qualityInspectionPlans = pgTable("quality_inspection_plans", {
  id: uuid("id").defaultRandom().primaryKey(),
  tenantId: uuid("tenant_id").references(() => platformTenants.id).notNull(),
  planCode: varchar("plan_code", { length: 80 }).notNull(),
  planName: varchar("plan_name", { length: 180 }).notNull(),
  objectType: varchar("object_type", { length: 80 }).notNull().default("work_order_execution"),
  processId: integer("process_id").references(() => processes.id),
  stepId: integer("step_id").references(() => processSteps.id),
  version: integer("version").notNull().default(1),
  status: varchar("status", { length: 30 }).notNull().default("draft"),
  triggerStatus: varchar("trigger_status", { length: 40 }).notNull().default("in_progress"),
  effectiveFrom: timestamp("effective_from"),
  effectiveTo: timestamp("effective_to"),
  checkpointIds: jsonb("checkpoint_ids").$type<number[]>(),
  createdBy: uuid("created_by").references(() => platformUsers.id),
  updatedBy: uuid("updated_by").references(() => platformUsers.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => [
  index("quality_inspection_plans_tenant_idx").on(table.tenantId),
  index("quality_inspection_plans_code_idx").on(table.tenantId, table.planCode),
  index("quality_inspection_plans_process_idx").on(table.processId),
  index("quality_inspection_plans_status_idx").on(table.tenantId, table.status),
]);

export const oeeReasonCodes = pgTable("oee_reason_codes", {
  id: uuid("id").defaultRandom().primaryKey(),
  tenantId: uuid("tenant_id").references(() => platformTenants.id).notNull(),
  reasonCode: varchar("reason_code", { length: 80 }).notNull(),
  reasonName: varchar("reason_name", { length: 180 }).notNull(),
  category: varchar("category", { length: 80 }).notNull().default("downtime"),
  lossType: varchar("loss_type", { length: 80 }).notNull().default("availability"),
  workCenterType: varchar("work_center_type", { length: 80 }),
  ...
  index("oee_reason_codes_tenant_idx").on(table.tenantId),
  index("oee_reason_codes_code_idx").on(table.tenantId, table.reasonCode),
  index("oee_reason_codes_category_idx").on(table.tenantId, table.category),
]);
